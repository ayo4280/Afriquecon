    ...
    <div className="bg-[#F4F6FA] min-h-screen py-10">
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        >
          ...
          {t('passengerBooking.backToSearch')}
        </button>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 mb-8 flex flex-col md:flex-row justify-between items-center gap-5 animate-fade-up">
          <div className="flex items-center gap-4">
            <div className="text-center">
              <div className="flex items-center gap-1 text-slate-400 text-xs mb-1">
                <MapPin className="w-3 h-3" /> {t('passengerBooking.from')}
              </div>
            ...
            <div className="flex flex-col items-center gap-1 px-4">
              ...
              <span className="text-xs text-slate-400 font-medium">{t('passengerBooking.direct')}</span>
            </div>
            <div className="text-center">
              <div className="flex items-center gap-1 text-slate-400 text-xs mb-1">
                <MapPin className="w-3 h-3" /> {t('passengerBooking.to')}
              </div>
          ...
          <div className="flex gap-5 text-slate-500 font-medium text-sm">
            ...
            <div className="flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
              ...
              {t('passengerBooking.passengerCount', { count: passengers })}
            </div>
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-slate-500 font-medium">{t('passengerBooking.searchingTrips')}</p>
          </div>
        ) : trips.length === 0 ? (
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-14 text-center animate-fade-up">
            ...
            <h3 className="text-2xl font-display font-bold text-slate-800 mb-2">{t('passengerBooking.noTrips')}</h3>
            <p className="text-slate-500 mb-6 max-w-sm mx-auto">
              {t('passengerBooking.noTripsDescription', { origin, destination })}
            </p>
            <button onClick={() => navigate('/')} className="bg-[#0A1628] text-white px-6 py-3 rounded-xl font-bold hover:bg-[#1a2d4e] transition-colors">
              {t('passengerBooking.modifySearch')}
            </button>
          </div>
        ) : (
          <div className="space-y-4 animate-fade-up">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-lg font-display font-bold text-slate-800">
                {t('passengerBooking.availableTrips', { count: trips.length })}
              </h2>
              <span className="text-xs text-slate-400 font-medium">{t('passengerBooking.passengerCount', { count: passengers })}</span>
            </div>
                  ...
                  <div className="p-6 flex flex-col md:flex-row items-center gap-6">
                    ...
                    <div className="flex-1 grid grid-cols-3 gap-4 items-center w-full">
                      ...
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <div className="text-xs text-slate-400 mt-2">{t('passengerBooking.direct')}</div>
                      </div>
