    ...
    <div className="flex-grow flex items-center justify-center py-12 px-6 bg-[#F4F6FA]">
      <div className="w-full max-w-md bg-white rounded-3xl p-8 sm:p-10 shadow-xl border border-slate-100">
        <Link to="/login" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-slate-800 mb-8">
          <ArrowLeft className="w-4 h-4" /> {t('auth.forgotBack')}
        </Link>
        ...
        {sent ? (
          <div className="text-center">
            ...
            <h1 className="text-2xl font-display font-bold text-slate-900 mb-3">{t('auth.forgotEmailSentTitle')}</h1>
            <p className="text-slate-500 leading-relaxed">{t('auth.forgotEmailSentMessage')}</p>
          </div>
        ) : (
          <>
            ...
            <h1 className="text-3xl font-display font-bold text-slate-900 mb-2">{t('auth.forgotTitle')}</h1>
            <p className="text-slate-500 mb-7">{t('auth.forgotSubtitle')}</p>
