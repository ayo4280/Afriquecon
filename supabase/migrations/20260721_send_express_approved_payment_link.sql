-- Send the secure payment link when management approves either a large
-- (100 kg+) or Express cargo booking. Express bookings have a manually set
-- negotiated price just like large cargo bookings.
...
DECLARE
  ...
  email_html text;
BEGIN
  -- Only approval-required cargo should receive this message, once, when it
  -- moves to confirmed with a negotiated amount.
  IF NOT (coalesce(NEW.is_express, false) OR coalesce(NEW.weight_kg, 0) >= 100)
     OR NEW.status IS DISTINCT FROM 'confirmed'
     OR OLD.status = 'confirmed'
     OR coalesce(NEW.total_fcfa, 0) <= 0 THEN
    RETURN NEW;
  END IF;

  UPDATE public.cargo_bookings
  SET approved_at = coalesce(approved_at, now()),
      approved_by = coalesce(approved_by, current_setting('request.jwt.claim.email', true))
  WHERE id = NEW.id;
  ...
  IF brevo_api_key IS NOT NULL AND nullif(NEW.customer_email, '') IS NOT NULL THEN
    ...
    PERFORM net.http_post(
      ...
      body := jsonb_build_object('sender', jsonb_build_object('name', 'Afriquecon', 'email', sender_email), 'to', jsonb_build_array(jsonb_build_object('email', NEW.customer_email)), 'subject', email_subject, 'htmlContent', email_html),
      timeout_milliseconds := 15000
    );
  END IF;
