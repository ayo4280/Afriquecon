-- Include a direct e-ticket link in passenger payment confirmation emails.
-- The QR itself is generated locally by the web app on the e-ticket page.
...
BEGIN
  ...
  app_url := rtrim(COALESCE(NULLIF(app_url, ''), 'https://www.afriquecon.com'), '/');
  ...
  IF TG_TABLE_NAME = 'passenger_tickets' THEN
    ...
    eticket_url := app_url || '/ticket/' || NEW.ticket_id;
    email_subject := 'Afriquecon: Your Ticket is Confirmed! (TKT: ' || NEW.ticket_id || ')';
    email_html := '<div style="font-family:sans-serif;max-width:600px;margin:0 auto">'
      ...
      || '<p><a href="' || eticket_url || '" style="display:inline-block;background:#f59e0b;color:#0A1628;padding:12px 18px;border-radius:8px;text-decoration:none;font-weight:bold">Open your e-ticket and QR code</a></p>'
      || '<p>Save or print the e-ticket and present the QR code at boarding. If the QR code is unavailable, staff can verify the Ticket ID manually.</p>'
      || '<p>Thank you for traveling with Afriquecon!</p></div>';
  ELSIF TG_TABLE_NAME = 'cargo_bookings' THEN
    customer_email := NEW.customer_email;
    IF customer_email IS NULL THEN RETURN NEW; END IF;

    email_subject := 'Afriquecon: Cargo Shipment Booked! (ID: ' || NEW.booking_id || ')';
    email_html := '<div style="font-family:sans-serif;max-width:600px;margin:0 auto">'
      ...
      || '<p>Thank you for choosing Afriquecon!</p></div>';
  ELSE
    RETURN NEW;
  END IF;
...
COMMENT ON FUNCTION public.notify_email_brevo() IS
  'Sends Afriquecon-branded payment confirmations; passenger emails include a direct e-ticket/QR link.';
