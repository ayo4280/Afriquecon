-- Send the approved-quote payment notification whenever a large cargo booking
-- transitions into the confirmed state. Managers may save the negotiated price
-- and the status in separate updates, so this must not depend on the prior price.
